#' Download Loan Fees
#'
#' Downloads and aggregates Loan Fees data from NEFIN's website.
...
#'
#' \dontrun{
#' df.out <- get_loan_fees(agg = agg, agg.func = agg.func)
#' }
get_loan_fees <- function(agg = 'daily',
                          agg.func = NULL,
                          progress = TRUE) {
  message('Collecting Loan Fees data')
  # URL and tmp file
  root.url <-  'http://www.nefin.com.br/Predictability/'
  url.file <- 'loan_fees.xls'
  # read excel
  # temp file
