...
#'
#' Downloads and aggregates Short Interest data from NEFIN's website.
...
#'
#' \dontrun{
#' df.out <- get_short_interest(agg = agg, agg.func = agg.func)
#' }
get_short_interest <- function(agg = 'daily',
                              agg.func = NULL,
                              progress = TRUE) {
  message('Collecting Short Interest data')
  # URL and tmp file
  root.url <-  'http://www.nefin.com.br/Predictability/'
  url.file <- 'short_interest.xls'
  # read excel
  # temp file
